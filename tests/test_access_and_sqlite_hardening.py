...
class AccessAndSqliteHardeningTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_web_session_rechecks_allowlist_for_authenticated_requests(self) -> None:
        db = SQLiteDatabase(self.db_path)
        await db.connect()
        ...
        user_service = UserService(users, access=AccessPolicy(settings), audit=audit)
        ...
        try:
            token = web_server._sessions.issue(telegram_user_id=202)
